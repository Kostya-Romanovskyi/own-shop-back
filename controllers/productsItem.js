...
const addIngredientsToItem = async (req, res) => {
  ...
  try {
    ...
    const responseMessage = {
      ...
      existing:
        existingIngredientDetails.length > 0
...
const removeIngredientsFromItem = async (req, res) => {
  ...
  try {
    ...
    const responseMessage = {
      message: "Ingredients processed successfully",
      removed:
        removedIngredientIds.length > 0 ? removedIngredientIds : undefined,
      notFound:
        notFoundIngredientDetails.length > 0
          ? notFoundIngredientDetails
          : undefined,
    };
    ...
    return res
      .status(500)
      .json({ error: "Error removing ingredients from item" });
  }
};
...
const addNewItem = async (req, res) => {
  try {
    ...
    if (existedName)
      return res.status(409).json({ message: `Item ${name} is already exist` });
...
const updateItem = async (req, res) => {
  try {
    ...
    if (name) {
      const existedName = await ProductsItem.findOne({ where: { name } });
      if (existedName)
        return res.status(409).json({ error: `Item ${name} is already exist` });
    }

    if (description) {
      const existedDescription = await ProductsItem.findOne({
        where: { description },
      });
      if (existedDescription)
        return res
          .status(409)
          .json({ error: `Item with this description is already exist` });
    }
    ...
    if (req.file) {
      ...
      fs.unlink(tempUpload, (error) => {
        if (error)
          return console.log("Error deleting the temporary file:", error);
        console.log("Temporary file successfully deleted ");
      });
    }

    await ProductsItem.update(
